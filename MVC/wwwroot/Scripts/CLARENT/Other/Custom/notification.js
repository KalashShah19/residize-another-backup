...
$(document).ready(function () {
    $(".dropdownnotification > a").each(function () {
        $(this).click(function () {
            $(".dropdownnotification").removeClass("open");
            $(this).parent(".dropdownnotification").toggleClass("open");
        });
    });

    $(document).on("click", function (event) {
        var $trigger = $(".dropdownnotification");
    ...
    $(document).on('click', '.dropdown-notification > li > a', function () {
        $('.dropdownnotification, .dropup').removeClass('open');
        $('.menu-dynamic').remove();
    });
    ...
    $('.notification').click(function () {
        ...
        $('.notification-menu.dropdown-notification').addClass('open');
    });
    ...
    $(document).on('click', function (e) {
        if (!$('.notification, .notification-menu').is(e.target)
            && $('.notification, .notification-menu').has(e.target).length === 0
    ...
    $(document).on('keydown', function (e) {
        if (e.keyCode === 27) { // ESC
            if ($('.notification-menu').hasClass('open') || $('.notification-wrapper').hasClass('open')) {
                closeUserprofile();
    ...
    $('.notification-menu .close').click(function () {
        closeUserprofile();
    });
