...
public class PropertyRepository : IPropertyRepository
{
    ...
    public void AddProperty(Properties.Post property, int propertyDetailsid)
    {
        ...
        FileHandler.StorePropertyVideo(property.PropertyVideos, GetCurrentPropertyId());
    ...
    {
        if (!IsPropertyExist(propertyDetails.PropertyId)) throw new Exception($"Property {propertyDetails.PropertyId} does not exist");
        NpgsqlCommand updatePropertyCommand = new("UPDATE t_properties SET c_property_name = @propertyname, c_property_amount = @propertyamount, c_property_square_ft = @propertysize, c_bhk = @bedrooms, c_bath = @bathrooms, c_floor = @floor, c_furnished = @furnished, c_inclusive_tax = @tax, WHERE c_project_id = @projectid RETURNING c_property_detail_id", connection);
        updatePropertyCommand.Parameters.AddWithValue("propertyname", propertyDetails.PropertyName);
        updatePropertyCommand.Parameters.AddWithValue("propertyamount", propertyDetails.PropertyAmount);
        updatePropertyCommand.Parameters.AddWithValue("propertysize", propertyDetails.PropertySize);
        updatePropertyCommand.Parameters.AddWithValue("bedrooms", propertyDetails.Bedrooms);
        updatePropertyCommand.Parameters.AddWithValue("bathrooms", propertyDetails.Bathrooms);
        updatePropertyCommand.Parameters.AddWithValue("floor", propertyDetails.Floor);
        updatePropertyCommand.Parameters.AddWithValue("furnished", propertyDetails.Furnished);
        updatePropertyCommand.Parameters.AddWithValue("tax", propertyDetails.TaxInclusion);
        updatePropertyCommand.Parameters.AddWithValue("c_property_id", propertyDetails.PropertyId);
        using NpgsqlDataReader reader = updatePropertyCommand.ExecuteReader();
        FileHandler.StorePropertyImages(propertyDetails.PropertyPictures, propertyDetails.PropertyId);
    ...
    public List<Properties.ListGet> GetProperties()
    {
		...
		from t_properties p  join t_details d on p.c_property_detail_id = d.c_property_detail_id left join t_projects pro on p.c_project_id = pro.c_project_id", connection);
    ...
    // public List<Amenities> GetAmenitiesByPropertyId()
    // {
    //     NpgsqlCommand AmenityCmd =
    // }
}
